...
export function FinancialHealthCard() {
    ...
    return (
        <Card className="h-full group hover:border-primary/50 hover:bg-primary/[0.01] hover:scale-[1.015] transition-all duration-500 shadow-premium glass-card overflow-hidden relative border-border/20">
            ...
            <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0 relative z-10 px-4 sm:px-6 pt-5">
                <div className="space-y-0.5">
                    <CardTitle className="text-[10px] font-black uppercase tracking-[0.25em] text-primary/60 flex items-center gap-2">
            ...
            <CardContent className="pt-0 flex flex-col items-center relative z-10 px-4 sm:px-6 pb-6">
                ...
                <div className="flex flex-row items-center justify-start gap-4 sm:gap-8 w-full">
                    {/* Compact Gauge Region */}
                    <div className="relative h-24 w-24 sm:h-32 sm:w-32 group-hover:scale-105 transition-transform duration-700 shrink-0">
                    ...
                    {/* Score Context Region */}
                    <div className="flex-1 min-w-0 pr-2">
                        <p className="text-2xl sm:text-3xl font-black tracking-tighter leading-none mb-1.5" style={{ color: scoreColor }}>
                            {getScoreTitle(scoreResult.score)}
                        </p>
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-primary/5 border border-primary/10 w-fit">
                             <ShieldCheck className="h-3 w-3 text-primary shrink-0" />
                             <span className="text-[9px] font-black uppercase tracking-tight truncate text-primary/80">Status: {scoreResult.savingsRatio > 0.1 ? 'Resilient' : 'Vulnerable'}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full mt-5 space-y-3">
                    <div className="flex justify-between items-end px-1">
                        <div className="space-y-0.5">
                             <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/30">Maturity Progress</p>
                             <p className="text-[12px] font-black tracking-tight">{scoreResult.goalAchievementRatio !== null ? `${(scoreResult.goalAchievementRatio * 100).toFixed(0)}%` : '0%'} Secured</p>
                        </div>
                        <Target className="h-4 w-4 text-primary/40" />
                    </div>
                    <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-muted/10 border border-white/5 shadow-inner p-0.5">
                        <div 
                            className="h-full bg-primary transition-all duration-1000 ease-out shadow-[0_0_12px_rgba(var(--primary-rgb),0.5)] rounded-full"
