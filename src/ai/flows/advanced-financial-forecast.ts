...
export async function generateAdvancedForecast(input: AdvancedForecastInput): Promise<AdvancedForecastOutput> {
  ...
  const promptInput = {
    ...
    incomeSources: JSON.stringify(input.incomeSources, null, 2),
    expenses: JSON.stringify(input.expenses, null, 2),
    budgets: JSON.stringify(input.budgets, null, 2),
    savingsGoals: JSON.stringify(input.savingsGoals, null, 2),
  };
  return advancedFinancialForecastFlow(promptInput);
}
