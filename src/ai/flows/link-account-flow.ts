...
// This is now a standard Next.js Server Action, removing Genkit from this specific process.
export async function exchangeTokenForAccount(input: ExchangeTokenInput): Promise<ExchangeTokenOutput> {
  const { firestore } = initializeFirebase();
  if (!firestore) {
      console.error("Account linking failed: Firestore is not initialized. Check server configuration for FIREBASE_SERVICE_ACCOUNT.");
